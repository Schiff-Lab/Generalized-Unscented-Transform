...
function [mu, second_cen_moment, third_cen_moment, ...
    fourth_cen_moment, Ex] =  Negative_Binomial_moments(r,p)
% INPUTS
%
...
mu = Ex1;    % = (r*(1 - p))/p
% Variance  --->   E[ (x - mu)^2 ]
second_cen_moment = simplify(Ex2 - mu*mu);  % (r*(1 - p))/p^2
% Skew      --->   E[ (x - mu)^3 ]
third_cen_moment = simplify(Ex3 - 3*Ex2*mu + 3*mu*mu^2 - mu^3); % (r*(p^2 - 3*p + 2))/p^3
% Kurtosis  --->   E[ (x - mu)^4 ]
fourth_cen_moment = simplify(Ex4 - 4*Ex3*mu + 6*Ex2*mu^2 - 4*mu*mu^3 + mu^4); % -(r*(p - 1)*(3*r - 6*p - 3*p*r + p^2 + 6))/p^4
