...
function [mu, second_cen_moment, third_cen_moment, ...
    fourth_cen_moment, Ex] =  Exponential_moments(lambda)
% INPUTS
%
%   lambda                      -   Mean of random variable
%
% OUTPUTS
%
...
mu = Ex1;    % = 1/lambda
% Variance  --->   E[ (x - mu)^2 ]
second_cen_moment = simplify(Ex2 - mu*mu);  % 1/lambda^2
% Skew      --->   E[ (x - mu)^3 ]
third_cen_moment = simplify(Ex3 - 3*Ex2*mu + 3*mu*mu^2 - mu^3); % 2/lambda^3
% Kurtosis  --->   E[ (x - mu)^4 ]
fourth_cen_moment = simplify(Ex4 - 4*Ex3*mu + 6*Ex2*mu^2 - 4*mu*mu^3 + mu^4); % 9/lambda^4
