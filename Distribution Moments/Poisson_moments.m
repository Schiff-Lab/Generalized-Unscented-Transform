...
function [mu, second_cen_moment, third_cen_moment, ...
    fourth_cen_moment, Ex] =  Poisson_moments(lambda)
% INPUTS
%
...
% syms lambda t
syms t
J = exp(lambda*(exp(t)-1)); %Poisson MGF
M1 = diff(J,t);
M2 = diff(M1,t);
M3 = diff(M2,t);
M4 = diff(M3,t);
...
mu = Ex1;    % = lambda
% Variance  --->   E[ (x - mu)^2 ]
second_cen_moment = simplify(Ex2 - mu*mu);  % lambda
% Skew      --->   E[ (x - mu)^3 ]
third_cen_moment = simplify(Ex3 - 3*Ex2*mu + 3*mu*mu^2 - mu^3); % lambda
% Kurtosis  --->   E[ (x - mu)^4 ]
fourth_cen_moment = simplify(Ex4 - 4*Ex3*mu + 6*Ex2*mu^2 - 4*mu*mu^3 + mu^4); % lambda*(3*lambda + 1)
